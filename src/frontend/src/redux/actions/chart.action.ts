...
import { CHART_QUERY_NAME, getChartQuery } from "gql/queries/chart.query"
...
export const getChartData = (interval: IntervalType) => async (dispatch: any, getState: any) => {
  try {
    ...
    const parsedChartDataToCandlestick = chartData?.[interval].map((chartPoint: ChartPoint) => ({
    ...
    const parsedChartDataToArea = chartData?.[interval].map((chartPoint: ChartPoint) => ({
      x: new Date(chartPoint.bucket),
